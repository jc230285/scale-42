...
function run() {
  ...
  for (const { file, lang } of dcPages) {
    ...
    if (lang === 'en') {
      ...
      html = updateMarker(html, 'dc_hero_lede', `${stats.projects} Data Centres across ${stats.countries} Nordic countries, co-located with low-cost renewable power, purpose-built for high-density compute. Scaling from 30 MW to 500 MW.`);
    } else {
      html = updateMarker(html, 'dc_hero_title', `${stats.pipeline} KI-klar nordisk kapasitet.`);
      html = updateMarker(html, 'dc_hero_lede', `${stats.projects} datasentre i ${stats.countries} nordiske land, samlokalisert med rimelig fornybar kraft, spesialbygget for høytetthetsberegning. Skalerer fra 30 MW til 500 MW.`);
    }
